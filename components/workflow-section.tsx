import { Database, Download, Terminal, CheckCircle2, ArrowRight } from "lucide-react";
...
const steps = [
  {
    ...
    description: "Get SQL Studio directly from the Play Store. It's a lightweight package optimized for performance and battery efficiency.",
    color: "from-blue-500 to-cyan-500"
  },
  {
    icon: Database,
    title: "Initialize Database",
    description: "Open an existing SQLite file or create a brand new database schema in seconds. No complex configuration required.",
    color: "from-cyan-500 to-teal-400"
  },
  {
    icon: Terminal,
    title: "Start Querying",
    description: "Write SQL with our smart editor. Execute queries, visualize results in tables, and export data with a single tap.",
...
export const WorkflowSection = () => {
  return (
    <section id="workflow" className="relative py-24 md:py-32 bg-white overflow-hidden">
      
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#f8fafc_1px,transparent_1px),linear-gradient(to_bottom,#f8fafc_1px,transparent_1px)] bg-size-[40px_40px] mask-[radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)]"></div>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-24">
          ...
          <div className="lg:w-1/2">
            ...
            <p className="text-lg text-slate-500 font-light mb-16 max-w-lg leading-relaxed">
              We stripped away the complexity of desktop clients. Get straight to querying with a streamlined mobile experience designed for speed.
            </p>

            <div className="relative space-y-12">
              <div className="absolute left-[27px] top-4 bottom-4 w-0.5 bg-linear-to-b from-slate-200 via-slate-200 to-transparent"></div>

              {steps.map((step, index) => (
                <div key={index} className="relative flex gap-8 group">
                  <div className="relative shrink-0">
                    <div className={`w-14 h-14 rounded-2xl bg-white border border-slate-200 shadow-sm flex items-center justify-center relative z-10 group-hover:scale-110 transition-transform duration-300`}>
                      <div className={`absolute inset-0 bg-linear-to-br ${step.color} opacity-0 group-hover:opacity-10 rounded-2xl transition-opacity duration-300`}></div>
          ...
          <div className="lg:w-1/2 relative w-full flex justify-center lg:justify-end">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-cyan-400/20 blur-[120px] rounded-full w-[150%] h-[150%] pointer-events-none"></div>
